...
#     )
# except ImportError:  # pragma: no cover - 모듈 경로 유연성 확보
#     from cook_session import session_manager, constraint_parser  # type: ignore
#     from rag_prompt_builder import (  # type: ignore
#         create_system_prompt,
#         create_step_prompt,
#         create_next_step_user_prompt,
#         constraints_to_dict_list,
...
# def normalize_ingredients(ingredients_field):
#     """문자열/리스트형 재료 필드를 리스트로 정규화"""
...
#         try:
#             recipe_id = selected_recipe_data.get("id", 0) or 0
#         except AttributeError:
#             recipe_id = 0
#         session = session_manager.create_session(USER_ID, recipe_id)
#         session.recipe_data = recipe_json
...
#                 bot_response = None
#                 recipe_steps = session.recipe_data.get("steps", []) if session and session.recipe_data else state["steps"]
#                 current_step = session.current_step if session else state["current_step"]

#                 if current_step < len(recipe_steps):
#                     step_content = recipe_steps[current_step]
#                     if session and session.recipe_data:
...
#                             bot_response = await call_llm(system_prompt, user_prompt)
#                         except Exception as e:
#                             logger.warning(f"개인화 단계 안내 생성 실패, 기본 문구 사용: {e}")
#                             bot_response = f"{current_step + 1}단계. {step_content}"
#                     else:
#                         bot_response = f"{current_step + 1}단계. {step_content}"
...
#             else:
#                 # System Prompt: 개인화 요구사항 반영
#                 if session and session.recipe_data:
#                     constraints = constraints_to_dict_list(session.constraints)
#                     system_prompt = create_system_prompt(session.recipe_data, constraints)
#                 else:
#                     system_prompt = (
...
#                     bot_response = "현재 답변을 생성할 수 없습니다. 잠시 후 다시 시도해주세요."
...
async def call_llm(system_prompt: str, user_prompt: str):
    ...
    llm_payload = {
        "prompt": final_prompt,
        "max_new_tokens": 256,
        "temperature": 0.2,     # 사실 기반 답변 유도 (환각 방지)
        "top_p": 0.9,
        "repetition_penalty": 1.1 
    }
